...
{
    SChatLog(@"patchFonts called with fonts: %@ and def name: %@", mainFonts, fontDefName);

    if (!fontMap)
    {
        SChatLog(@"Creating new fontMap");
    ...
    if (fontJson)
    {
        SChatLog(@"Found existing fonts.json: %@", fontJson);
    }

    for (NSString *fontName in mainFonts)
    {
        NSString *url = mainFonts[fontName];
        SChatLog(@"Replacing font %@ with URL: %@", fontName, url);
        ...
        {
            SChatLog(@"Creating parent directory: %@", parentDir.path);
        ...
        {
            SChatLog(@"Downloading font %@ from %@", fontName, url);
            NSData *data = [NSData dataWithContentsOfURL:fontURL];
            if (data)
            {
                SChatLog(@"Writing font data to: %@", fontCachePath.path);
        ...
        if (fontData)
        {
            SChatLog(@"Registering font %@ with provider", fontName);
            ...
            if (font)
            {
                ...
                if (existingFont)
                {
                    ...
                    {
                        SChatLog(@"Failed to deregister font %@: %@",
                ...
                if (CTFontManagerRegisterGraphicsFont(font, &error))
                {
                    ...
                    SChatLog(@"Successfully registered font %@ to %@", fontName,
                             (__bridge NSString *) postScriptName);
                ...
                else
                {
                    NSString *errorDesc = error
                                              ...
                                              : @"Unknown error";
                    SChatLog(@"Failed to register font %@: %@", fontName, errorDesc);
                    if (error)
                        CFRelease(error);
                }
...
%ctor
{
    ...
    {
        fontMap = [NSMutableDictionary dictionary];
        SChatLog(@"Font hooks initialized");
        %init;
    }
}
